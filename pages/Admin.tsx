        ...
        <div className="glass-panel w-full max-w-md p-8">
            ...
            {!isAuthenticated ? (
                <form onSubmit={handleLogin} className="space-y-6">
                    ...
                    {msg && <p className="text-brandRed-500 text-center text-xs mt-2 font-bold">{msg}</p>}
                    
                    <button onClick={() => navigate('/')} type="button" className="w-full text-center text-xs text-neutral-400 mt-4">
                        חזרה
                    </button>
                </form>
            ) : (
                <form onSubmit={handleUpdate} className="space-y-8">
                    <div className="bg-green-50 p-3 rounded text-center text-xs text-green-800 font-bold">
                        מחובר כמנהל
                    </div>

                    <div>
                        <label className="block text-xs font-bold text-neutral-500 mb-2">קוד רישום חדש</label>
                        <input 
                            type="text" 
                            value={newCode}
                    ...
                    <div className="border-t border-neutral-100 pt-6 mt-2">
                        <button onClick={() => navigate('/')} type="button" className="w-full text-center text-neutral-500 font-bold text-xs">יציאה</button>
                    </div>
                </form>
            )}
        </div>
